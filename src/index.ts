...
import OnlyForDisplay from "./controllers/OnlyForDisplay.controller";
...
//function to call only for display functions meant for swagger reference
OnlyForDisplay();
...
app.use("/sns", NotificationController())
...
app.use("/report", new ReportController().router);
...
app.get("/list", async (req, res) => {
  ...
  let x = r.Contents.map((item:any) => {item.Key});
  res.send(x)
})
